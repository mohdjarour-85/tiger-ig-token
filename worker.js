...
async function runScheduledPosts(env) {
  ...
  for (const post of results) {
    try {
      const mediaUrl = `https://${WORKER_HOST}/img/${post.media_key}`;
      ...
      const containerParams = isVideo
        ? { media_type: "REELS", video_url: mediaUrl, caption: post.caption || "", access_token: env.IG_ACCESS_TOKEN }
        : { image_url: mediaUrl, caption: post.caption || "", access_token: env.IG_ACCESS_TOKEN };
